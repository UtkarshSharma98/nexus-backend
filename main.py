...
from google.genai import types  # Added for strict schema compliance
import uvicorn
import razorpay
...
async def get_player_stats(user: Annotated[dict, Depends(get_current_user)]):
    ...
    if doc_snap.exists:
        ...
        # FIX: Validate that the active skill tree matches the user's current stream sector
...
async def create_order(payload: PurchaseSchema, user: Annotated[dict, Depends(get_current_user)]):
    ...
    try:
        order_data = {"amount": price_matrix[item]["amount"], "currency": "INR", "receipt": f"rcpt_{user['uid'][:10]}_{item[:5]}", "notes": {"player_uid": user["uid"], "purchased_item": item}}
        razorpay_order = client.order.create(data=order_data)
        return {"order_id": razorpay_order["id"], "amount": razorpay_order["amount"], "currency": razorpay_order["currency"], "key_id": RAZORPAY_KEY_ID, "product_name": price_matrix[item]["name"]}
    except Exception as e:
...
# ⚔️ THE CORE MISSION-GUARDED UNLOCK ENDPOINT
...
async def unlock_skill_tree_node(payload: UnlockNodeSchema, user: Annotated[dict, Depends(get_current_user)]):
    ...
    # Enforce active mission locking parameters
...
async def complete_mission(
    ...
    if payload.mission_id not in completed_missions:
        ...
        # Give a bonus reward for completing a main mission
...
async def generate_stream_mcq(payload: StudyMaterialRequestSchema, user: Annotated[dict, Depends(get_current_user)]):
    ...
    try:
        # Utilizing Structured Outputs Native Schema Enforcement
...
async def generate_stream_memory_cards(payload: StudyMaterialRequestSchema, user: Annotated[dict, Depends(get_current_user)]):
    ...
    try:
        ...
        response = gemini_client.models.generate_content(
            ...
            config=types.GenerateContentConfig(
                response_mime_type="application/json",
                response_schema=MemoryCardResponseSchema
            )
        )
        return json.loads(response.text)
    except Exception as e:
